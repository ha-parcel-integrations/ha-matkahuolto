"""Tests for Matkahuolto sensor property logic."""
from datetime import datetime, timezone
from unittest.mock import MagicMock

from custom_components.matkahuolto.const import ParcelStatus
from custom_components.matkahuolto.sensor import (
    MatkahuoltoAwaitingPickupSensor,
    MatkahuoltoDeliveredParcelsSensor,
    MatkahuoltoIncomingParcelsSensor,
    MatkahuoltoLastUpdateSensor,
    MatkahuoltoNextDeliverySensor,
    MatkahuoltoParcelSensor,
)
...
def test_incoming_counts_and_lists():
    ...
    sensor = MatkahuoltoIncomingParcelsSensor(coordinator, _entry(), lambda _: None, set())
...
def test_parcel_sensor_status_and_attributes():
    ...
    sensor = MatkahuoltoParcelSensor(_coordinator([parcel]), _entry(), "A")
...
def test_parcel_sensor_missing_barcode():
    sensor = MatkahuoltoParcelSensor(_coordinator([_parcel("A")]), _entry(), "OTHER")
...
def test_next_delivery_picks_earliest():
    ...
    sensor = MatkahuoltoNextDeliverySensor(coordinator, _entry())
...
def test_next_delivery_none_without_moments():
    sensor = MatkahuoltoNextDeliverySensor(_coordinator([_parcel("A")]), _entry())
    assert sensor.native_value is None
    assert sensor.extra_state_attributes == {}
...
def test_awaiting_pickup_counts_only_pickup_point_status():
    coordinator = _coordinator([
    ...
    sensor = MatkahuoltoAwaitingPickupSensor(coordinator, _entry())
    assert sensor.native_value == 1
    assert sensor.extra_state_attributes["parcels"][0]["barcode"] == "B"


def test_delivered_sensor():
    coordinator = _coordinator([], delivered=[_parcel("D", status=ParcelStatus.DELIVERED)])
    sensor = MatkahuoltoDeliveredParcelsSensor(coordinator, _entry())
    assert sensor.native_value == 1
