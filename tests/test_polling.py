...
from custom_components.matkahuolto.coordinator import (
    MatkahuoltoCoordinator,
...
async def test_update_interval_resumes_once_a_parcel_is_added(hass):
    """Matkahuolto never reports ``out_for_delivery`` — the tier resumes at
    mid, not hot; ``test_tier_is_hot_when_out_for_delivery_without_planned_from``
    above already covers the shared coordinator's hot path directly."""
    entry = _entry_with([])
    entry.add_to_hass(hass)
    client = AsyncMock()
    coordinator = MatkahuoltoCoordinator(hass, client, entry)
...
async def test_429_raises_update_failed_with_retry_after(hass):
    ...
    entry.add_to_hass(hass)
    client = AsyncMock()
    client.async_get_parcel.side_effect = MatkahuoltoApiError(
