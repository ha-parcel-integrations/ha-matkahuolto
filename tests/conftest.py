"""pytest configuration for the Matkahuolto test suite."""
import sys
...
def auto_enable_custom_integrations(enable_custom_integrations):
    """Make ``custom_components.matkahuolto`` loadable from config-flow / setup tests."""
    yield
...
def reset_one_shot_warnings():
    ...
    ``parcels._unmapped_descriptions_logged`` is module-level, so without this
    a test's one-shot-warning assertion would depend on which test already
    triggered that description first.
    """
    from custom_components.matkahuolto import parcels

    parcels._unmapped_descriptions_logged.clear()
    yield
