"""Tests for Matkahuolto setup and unload."""
...
from .payloads import in_transit_sample as _sample
...
async def test_setup_and_unload(hass):
    ...
    with patch(
        "custom_components.matkahuolto.api.MatkahuoltoApiClient.async_get_parcel",
    ...
    incoming = hass.states.get("sensor.matkahuolto_incoming_parcels")
...
async def test_setup_retries_when_first_refresh_fails(hass):
    ...
    with patch(
        "custom_components.matkahuolto.api.MatkahuoltoApiClient.async_get_parcel",
        new=AsyncMock(side_effect=MatkahuoltoApiError("Matkahuolto unreachable")),
    ):
...
async def test_per_parcel_sensor_spawn_and_remove(hass):
    ...
    with patch("custom_components.matkahuolto.api.MatkahuoltoApiClient.async_get_parcel", new=mock):
        assert await hass.config_entries.async_setup(entry.entry_id)
        await hass.async_block_till_done()
