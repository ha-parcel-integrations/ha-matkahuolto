# Automatic parcel tracking from e-mail (IMAP → Matkahuolto)

Companion guide for [`track_parcels_from_email.yaml`](track_parcels_from_email.yaml): watch your mailbox(es) for shipping e-mails, extract the Matkahuolto tracking code, and register it with `matkahuolto.track_parcel` — fully automatic, no extra custom component required.

Matkahuolto is a **code-based** carrier: it has no account inbox, so every parcel must be registered by its tracking code before the integration can follow it. This recipe automates exactly that step.

**How it works, in one line:** the core [IMAP integration](https://www.home-assistant.io/integrations/imap/) fires an `imap_content` event for every new e-mail (including the body); the automation extracts the tracking code — a cheap regex first, an optional AI fallback for everything else — and calls `matkahuolto.track_parcel`.

```
new e-mail ──imap_content──▶ automation ──▶ regex match? ──▶ matkahuolto.track_parcel
                                     │
...
- This integration, with the `matkahuolto.track_parcel` action available (field `tracking_code`).
...
The regex matches either of the two shapes seen in the wild:

```
(?<![0-9A-Za-z])(?:MH[0-9]{6,}FI|[0-9]{20})(?![0-9A-Za-z])
```

**`MH…FI`** (e.g. `MH900000001FI`) and a bare **20-digit SSCC** (e.g. `00000012345678901234`). The integration itself accepts any non-empty code, but matching that broadly against e-mail text would hit every order number in the mail — so the example anchors on the two known shapes instead. The SSCC has no letter prefix, so it is matched on its exact length (20 digits), which is specific enough in practice not to catch phone numbers or order IDs. **If your mails carry a different shape, add it to the alternation**; the AI fallback will surface unknown formats in the meantime.
...
curl -X POST -H "Authorization: Bearer $HA_TOKEN" -H "Content-Type: application/json" \
  ...
  -d '{"sender":"noreply@matkahuoltoshipping.com","subject":"Your parcel is on its way",
       "text":"Tracking code: MH123456789FI","initial":true,"folder":"INBOX","username":"test"}'
```

Then `matkahuolto.untrack_parcel` the test code afterwards. For a full end-to-end test, forward a real shipping mail to the watched mailbox — it must arrive **unread**.
