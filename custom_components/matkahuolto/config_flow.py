"""Config flow for the Matkahuolto parcel tracker integration."""
...
class MatkahuoltoConfigFlow(ConfigFlow, domain=DOMAIN):
    """Handle the UI-driven configuration flow for the Matkahuolto integration."""
    ...
    ) -> MatkahuoltoOptionsFlowHandler:
        """Return the options flow handler."""
        return MatkahuoltoOptionsFlowHandler()

    async def async_step_user(
        self, user_input: dict[str, Any] | None = None
    ) -> ConfigFlowResult:
        """Create the Matkahuolto hub — single instance, no input needed.

        Tracking is keyed on the tracking code alone (no account, no postal
        code — Matkahuolto's anonymous endpoint takes only ``parcelNumber``),
        so there is nothing to ask at setup: the entry is created straight
        away and parcels are added afterwards via the options flow, the
        ``matkahuolto.track_parcel`` service or a dashboard button.
...
class MatkahuoltoOptionsFlowHandler(OptionsFlow):
