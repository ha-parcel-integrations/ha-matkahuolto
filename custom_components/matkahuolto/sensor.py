"""Sensor platform for the Matkahuolto parcel tracker integration."""
...
from .coordinator import MatkahuoltoCoordinator
...
async def async_setup_entry(
    ...
    """Set up Matkahuolto sensor entities from a config entry."""
    ...
    entities: list[SensorEntity] = [
        MatkahuoltoIncomingParcelsSensor(
            coordinator, entry, async_add_entities, current_barcodes
        ),
    ]
    for parcel in coordinator.data or []:
        entities.append(
            MatkahuoltoParcelSensor(coordinator, entry, parcel.get("barcode", ""))
        )
    entities.append(MatkahuoltoNextDeliverySensor(coordinator, entry))
    entities.append(MatkahuoltoAwaitingPickupSensor(coordinator, entry))
    entities.append(MatkahuoltoDeliveredParcelsSensor(coordinator, entry))
    entities.append(MatkahuoltoLastUpdateSensor(coordinator, entry))

    async_add_entities(entities)


class MatkahuoltoIncomingParcelsSensor(
    CoordinatorEntity[MatkahuoltoCoordinator], SensorEntity
):
    ...
    def __init__(
        self,
        coordinator: MatkahuoltoCoordinator,
        entry: ConfigEntry,
        async_add_entities: AddEntitiesCallback,
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if new_barcodes:
            self._async_add_entities(
                MatkahuoltoParcelSensor(self.coordinator, self._entry, barcode)
...
class MatkahuoltoParcelSensor(CoordinatorEntity[MatkahuoltoCoordinator], SensorEntity):
    """Per-parcel sensor reporting the status of one tracked Matkahuolto parcel."""
    ...
    def __init__(
        self, coordinator: MatkahuoltoCoordinator, entry: ConfigEntry, barcode: str
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
        self._entry = entry
...
class MatkahuoltoNextDeliverySensor(
    CoordinatorEntity[MatkahuoltoCoordinator], SensorEntity
):
    ...
    def __init__(
        self, coordinator: MatkahuoltoCoordinator, entry: ConfigEntry
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
...
class MatkahuoltoAwaitingPickupSensor(
    CoordinatorEntity[MatkahuoltoCoordinator], SensorEntity
):
    """Parcels that have arrived at a Matkahuolto pickup point and are ready to collect."""
...
class MatkahuoltoDeliveredParcelsSensor(
    CoordinatorEntity[MatkahuoltoCoordinator], SensorEntity
):
    """Recently delivered tracked Matkahuolto parcels."""
...
class MatkahuoltoLastUpdateSensor(
    CoordinatorEntity[MatkahuoltoCoordinator], SensorEntity
):
    """Diagnostic sensor reporting when Matkahuolto was last polled successfully."""
