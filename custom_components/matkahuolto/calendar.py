"""Calendar platform for the Matkahuolto parcel tracker integration."""
...
async def async_setup_entry(
    ...
    """Set up the Matkahuolto deliveries calendar from a config entry."""
    async_add_entities([MatkahuoltoDeliveriesCalendar(entry.runtime_data.coordinator, entry)])


class MatkahuoltoDeliveriesCalendar(CoordinatorEntity[MatkahuoltoCoordinator], CalendarEntity):
    """A read-only calendar of expected Matkahuolto deliveries.
    ...
    def __init__(self, coordinator: MatkahuoltoCoordinator, entry: ConfigEntry) -> None:
    ...
    def _events(self) -> list[CalendarEvent]:
        ...
        for parcel in self.coordinator.data or []:
            ...
            summary = sender or (f"Parcel {barcode}" if barcode else "Matkahuolto parcel")
            description_parts = [
