"""Matkahuolto parcel tracker custom component for Home Assistant."""
...
from .api import MatkahuoltoApiClient
...
@dataclass
class MatkahuoltoData:
    """Runtime data attached to the Matkahuolto config entry."""

    client: MatkahuoltoApiClient
    coordinator: MatkahuoltoCoordinator


type MatkahuoltoConfigEntry = ConfigEntry[MatkahuoltoData]


async def async_setup_entry(hass: HomeAssistant, entry: MatkahuoltoConfigEntry) -> bool:
    """Set up Matkahuolto from a config entry."""
    # No auth: Matkahuolto tracking is public, so the HA-managed session is fine.
    client = MatkahuoltoApiClient(async_get_clientsession(hass))
    ...
    entry.runtime_data = MatkahuoltoData(client=client, coordinator=coordinator)
...
async def async_unload_entry(hass: HomeAssistant, entry: MatkahuoltoConfigEntry) -> bool:
    """Unload the Matkahuolto config entry."""
